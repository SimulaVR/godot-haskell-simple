...
-- Question for David: How do we know if the  `godot_gdnative_core_api_struct` and `godot_gdnative_ext_nativescript_api_struct` are cleaned up? Does godot-haskell do this for us automatically?
...
godot_nativescript_init desc = do
  ...
  -- Below we use simplified wrapper functions for `godot_nativescript_register_class` and `godot_nativescript_register_method`.
...
-- Question: What happens to the GodotString? In the C code we had to manually delete it via `godot_string_destroy`. (What about GodotVariants)?
...
simple_get_data _ t@(UserData obj ioRefStr) _ = do
  ...
  return userDataGodotVariant

-- We finally export the core godot_* functions so that NativeScript can call them automatically when instances of our SIMPLE script are loaded.
